...
| 目录 / appid | 说明 |
|---|---|
| `office` | OnlyOffice 在线文档编辑（官方镜像 + 挂载定制资源；最复杂，有专门的 add-version 技能） |
| `drawio` | drawio 流程图 |
| `face` | 人脸相关 |
| `fileview` | 文件预览 |
| `minder` | 思维导图 |
...
.claude/skills/
├── add-version/                # 仅 office：从官方镜像提取并套用定制，生成新版本目录
...
- **office 升级 OnlyOffice 版本**：用 `add-version` 技能（pull 指定版镜像 → 提取资源 → 套用定制 → 生成新版本目录），其定制片段以「干净容器 vs 现有版本逐文件 diff」实测固化，可字节级复现。
