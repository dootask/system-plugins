...
| `drawio` | 纯打包 | drawio 流程图（`jgraph/drawio` + 导出服务 `kuaifan/export-server`） |
| `fileview` | 纯打包 | 文件预览（`kuaifan/fileview`，镜像在他处构建） |
| `mysql-expose-port` | 纯打包 | MySQL 端口暴露（`nginx:alpine` TCP 代理） |
| `search` | 纯打包 | Manticore 搜索（官方 `manticoresearch/manticore`） |
| `ai` | 构建镜像 | DooTask AI（Python 应用，构建 `dootask/ai`） |
| `approve` | 构建镜像 | 审批中心（Go + Vue3 应用，构建 `dootask/approve`） |
| `okr` | 构建镜像 | OKR 目标管理（Go + Vue3，构建 `dootask/okr`；源码来自 [hitosea/dootask-okr](https://github.com/hitosea/dootask-okr)） |
| `face` | 构建镜像 | 人脸签到（Go 应用，构建 `dootask/face`；源码来自 [innet8/doofacemark](https://github.com/innet8/doofacemark)） |
| `minder` | 构建镜像 | 思维导图（Vue 应用，构建 `dootask/minder`；源码来自 [kuaifan/minder](https://github.com/kuaifan/minder)） |

- **纯打包 / 资源挂载**：发布时只打包目录，不构建镜像。
- **构建镜像**：插件目录含源码（在 `src/`）与 `.build.yml`，发布时先构建并推送 Docker 镜像，再打包发布。
- 仍引用「他处构建」镜像、源码不在本仓库的：`drawio` 的 `kuaifan/export-server`、`fileview` 的 `kuaifan/fileview`。
...
- **ai / approve 改代码**：改 `src/` 下源码，按新版本号新建 `<版本>/` 目录（更新其 CHANGELOG），推 `<插件>/<新版本>` tag —— 工作流会用该版本号构建镜像并发布。
...
| `DOCKER_USERNAME` / `DOCKER_PASSWORD` | Docker Hub 账号密码（仅 `ai`、`approve` 等含 `.build.yml` 的插件构建镜像时需要） |
