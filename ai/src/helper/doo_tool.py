...
- 插件  app list | catalog [--search 关键词] | fields <ID> | install <ID> [--version --param K=V --cpu-limit --memory-limit --pull]
        | update <ID> [同上] | reinstall <ID> [同上] | uninstall <ID> [--delete-data] | remove <ID>
        | logs <ID> | containers <ID> | refresh
        （应用插件管理；install/update/uninstall/remove 需管理员；
         用户描述需求时先 doo app catalog --search <关键词> 找应用ID（中英文均可，匹配 id/name/description/tags）；
         安装前先 doo app fields <ID> 查参数定义；
         已装应用未传 --param 时自动沿用当前值，不会清空令牌等）
