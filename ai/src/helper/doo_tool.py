...
- 插件  app list | catalog | fields <ID> | install <ID> [--version --param K=V --cpu-limit --memory-limit --pull]
        | update <ID> [同上] | reinstall <ID> [同上] | uninstall <ID> [--delete-data] | remove <ID>
        | logs <ID> | containers <ID> | refresh
        （应用插件管理；install/update/uninstall/remove 需管理员；安装前先 doo app fields <ID> 查参数定义；
