...
- 页面  page context [--query 词] | action <名> [--params JSON] | element <UID> <动作> [值]
        （页面操作驱动用户当前浏览器；context 默认采集用户最前面看到的页面——
         有微应用插件在最前打开时会采集并可操作其内部元素，否则采集主界面；
         跨源应用会提示不可操作，此时改用数据命令；会话失效只影响 page，数据命令不受影响）
- 插件运维  app list | updates | catalog [--search 词] | fields <ID> | upload <zip> [--appid]
        | install <ID> [--version --param K=V --cpu-limit --memory-limit --pull] | update <ID> [同上]
        | reinstall <ID> [同上] | uninstall <ID> [--delete-data] | remove <ID>
        | logs <ID> | containers <ID> | container-logs <ID> --service 名 | refresh
        （install/update/uninstall/remove/upload 需管理员；
         先 doo app catalog --search <词> 找应用ID（中英文均可，匹配 id/name/description/tags）；
         装前 doo app fields <ID> 查参数定义；已装应用未传 --param 自动沿用当前值，不会清空令牌等）
- 调用应用  app call <ID> [指令] [K=V ...]
        调用已安装应用的后端接口，执行其提供的业务能力（如审批/请假、CRM、表单、知识库等，不限于此）。
        不确定哪个应用能做、或不知应用ID/指令时：doo app catalog --search <词> / app list 找应用 →
        app call <ID> 列指令 → <指令> --describe 看参数 → <指令> K=V 调用（--data 传 JSON 体、--raw <METHOD> <path> 调未声明端点）。
        应用提供了对应能力就用 app call；没有时再用 page 在页面上操作。
