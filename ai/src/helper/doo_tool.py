...
- 任务  task list [--project ID --status uncompleted | completed --search 词] | view <ID> [--content --files]
        ...
        task update <ID> [--name --content --owner ...] | move <ID> [--project ID --column ID --flow ID] | done <ID> | undone <ID>
        task files <ID> | dialog <ID> | notify <ID> --text 内容 [--nickname 名 --silence] | archive <ID> | delete <ID>
- 项目  project list | view <ID> | create --name 名 | update <ID> [...] | exit <ID> | delete <ID>
- 看板列  column list --project ID | create --project ID --name 名 | update <列ID> [--name --color] | delete <列ID>
- 对话  dialog list | search <词> | view <ID> | users <ID> | inbox | mytodo [--dialog ID] | unread <ID> | read <ID> [--after 消息ID]
- 消息  message send --dialog ID --text 内容 [--silence] | send-user --user ID --text 内容
        message list --dialog ID | search --key 词 [--dialog ID] | view <ID> | withdraw <ID> | forward <ID>
        message todo <ID> | todolist <ID> | todo-remind <ID> --users ID [--at 时间] | done <待办ID>
- 群组  group create | edit <ID> | add-user <ID> | remove-user <ID> | exit <ID> | transfer <ID> | disband <ID>
- 用户  user info | departments | basic <id> [id...] | search <词>
- 机器人  bot list | view <ID> | create | update <ID> | delete <ID>
- 文件  file list | search <词> | view <ID> | fetch <路径> | history <ID>
        file create <名> [--type document --pid ID] | save <ID> [--content 文本 | --file 路径] [--format md | text] | rename <ID> <新名> | restore <ID> <历史ID> | delete <ID...>
        （save 仅支持 document；delete/restore 等写操作需 --yes）
- 报告  report unread | received | my | view <ID> | template | submit | analyze <报告ID> | share <报告ID> [报告ID...] | mark <报告ID> [报告ID...]
- 搜索  search <关键词> [--types task,project,file,...]
- 页面  page context [--query 词] | action <名> [--params JSON] | element <UID> <动作> [值]
        （页面操作驱动用户当前浏览器；context 默认采集用户最前面看到的页面——有微应用插件在最前打开时会采集并可操作其内部元素，否则采集主界面；跨源应用会提示不可操作，此时改用数据命令；会话失效只影响 page，数据命令不受影响）
- 插件运维  app list | updates | catalog [--search 词] | fields <ID> | upload <zip> [--appid]
        app install <ID> [--version --param K=V --cpu-limit --memory-limit --pull] | update <ID> [同上]
        app reinstall <ID> [同上] | uninstall <ID> [--delete-data] | remove <ID>
        app logs <ID> | containers <ID> | container-logs <ID> --service 名 | refresh
        （install/update/uninstall/remove/upload 需管理员；先 doo app catalog --search <词> 找应用 ID（中英文均可，匹配 id/name/description/tags）；装前 doo app fields <ID> 查参数定义；已装应用未传 --param 自动沿用当前值，不会清空令牌等）
- 调用应用  app call <ID> [指令] [K=V ...]
        调用已安装应用的后端接口，执行其提供的业务能力（如审批/请假、CRM、表单、知识库等，不限于此）。
        不确定哪个应用能做、或不知应用 ID/指令时：doo app catalog --search <词> / doo app list 找应用 →
        doo app call <ID> 列指令 → <指令> --describe 看参数 → <指令> K=V 调用（--data 传 JSON 体、--raw <METHOD> <path> 调未声明端点）。
        应用提供了对应能力就用 doo app call；没有时再用 doo page 在页面上操作。
