    ...
    <div
        ...
        <div class="md:flex-1 flex flex-col relative md:overflow-hidden bg-white px-16 pt-16 md:pt-0 md:px-0">
            <div class="flex max-md:hidden min-h-[44px] items-center justify-between pb-[15px] border-solid border-0 border-b-[1px] border-[#F2F3F5] relative md:mr-24">
                ...
                <img v-if="detailData.completed == '1'" class="absolute right-24 -bottom-[50px] z-[10]" src="@/assets/images/icon/complete.png" />
            </div>
        ...
        <div class="md:min-w-[35.8%] relative flex flex-col flex-1 md:flex-initial border-solid border-0 md:border-l-[1px] border-[#F2F3F5] ">
            ...
            <div v-if="loadIngR" class="absolute top-[71px] bottom-[24px] left-0 right-0 z-10 bg-[rgba(255,255,255,0.8)] flex-1 flex justify-center items-center">
                <n-spin size="small" />
            </div>
        </div>
...
const superiorUser = ref(0)
const inputShow = ref(false)
const canOwnerUpdateScore = ref(false)
const canSuperiorUpdateScore = ref(false)
...
const handleMark = (id, scores, superior_score, progress, can_owner_update_score, can_superior_update_score) => {
    ...
    if (userInfo.userid == detailData.value.userid || (detailData.value.superior_user?.indexOf(userInfo.userid) != -1 && detailData.value.superior_user?.indexOf(userInfo.userid) != undefined)) {
        ...
        //自己是负责人评完分的时候
        if (userInfo.userid == detailData.value.userid && scores > -1 && detailData.value.superior_user?.indexOf(userInfo.userid) != -1) {
            inputShow.value = false
        }
        //上级评完分的时候
...
const handleWarningShow = (e) => {
    ...
    if (e == 1) {
        OTitle.value = $t('删除')
        OContent.value = $t('确定要删除') + `【${detailData.value.title}】？`
        OType.value = e;
    }
    if (e == 2) {
        if (detailData.value.status == '1') {
